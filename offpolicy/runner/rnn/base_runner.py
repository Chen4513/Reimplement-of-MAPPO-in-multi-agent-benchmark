...
class RecRunner(object):
    ...
    def __init__(self, config):
        ...
        csv_path = f"/home/xiyue/mappo/onpolicy/scripts/train_mpe_scripts/{self.env_name}_{self.algorithm_name}_{now}.csv"

        self.csv_file = open(csv_path, "w", newline="")
        self.csv_writer = csv.writer(self.csv_file)

        # header
        self.csv_writer.writerow(["timestep", "average_episode_rewards"])
    ...
    def log_env(self, env_info, suffix=None):
        ...
        for k, v in env_info.items():
            if len(v) > 0:
                ...
                # if suffix_k == "average_episode_rewards":
                # scale = 16000 / (self.episode_length * self.num_envs)
                # scaled_step = self.total_env_steps * scale

                # self.csv_writer.writerow([scaled_step, v])
                self.csv_writer.writerow([self.total_env_steps, v])
                self.csv_file.flush()
                if self.use_wandb:
